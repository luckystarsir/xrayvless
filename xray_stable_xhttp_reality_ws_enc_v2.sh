...
if [ "$ENABLE_ENC" = "1" ]; then
  ...
  ENC_OUT=$(/usr/local/bin/xray vlessenc 2>/dev/null || true)

  VLESS_ENCRYPTION=$(echo "$ENC_OUT" | grep -i "Encryption" | grep -vi "Decryption" | head -n1 | cut -d ':' -f2- | sed 's/^ *//;s/[[:space:]]*$//')
  VLESS_DECRYPTION=$(echo "$ENC_OUT" | grep -i "Decryption" | head -n1 | cut -d ':' -f2- | sed 's/^ *//;s/[[:space:]]*$//')
