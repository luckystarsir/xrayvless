...
if [ "$ENABLE_ENC" = "1" ]; then
  echo ""
  echo "生成 VLESS Encryption / ENC 参数..."
  ENC_OUT=$(/usr/local/bin/xray vlessenc || true)

  VLESS_ENCRYPTION=$(echo "$ENC_OUT" | grep -i "Encryption" | grep -vi "Decryption" | head -n1 | cut -d ':' -f2- | tr -d '[:space:]')
  VLESS_DECRYPTION=$(echo "$ENC_OUT" | grep -i "Decryption" | head -n1 | cut -d ':' -f2- | tr -d '[:space:]')

  if [ -z "$VLESS_ENCRYPTION" ] || [ -z "$VLESS_DECRYPTION" ]; then
    echo "VLESS Encryption 参数生成失败，xray vlessenc 输出如下："
    echo "$ENC_OUT"
    exit 1
  fi
fi
...
2. REALITY 现在是 type=xhttp，不是 tcp。
3. ENC 启用时，客户端必须支持 Xray VLESS Encryption。
4. 如果客户端不支持 ENC，把链接里的 encryption=... 改成 encryption=none，并重新运行脚本选择不启用 ENC。
